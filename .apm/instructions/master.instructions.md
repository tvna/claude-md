...
- When that decision is genuinely the user's to make and you are outside plan mode, surface it through the harness's structured single-select question tool when one exists (Claude Code's AskUserQuestion is such a tool) rather than a free-form chat prompt, so the options stay inspectable at a glance; fall back to an explicitly enumerated chat question only where no such tool is available.

...
- For GitHub operations, prefer `gh` CLI (via Bash) for any GitHub MCP tool that has no `PreToolUse` hook entry in the agent settings. Only use GitHub MCP tools when a `PreToolUse` hook is in place to gate or audit that specific operation.

...
- You MUST write operator-facing output (chat responses in every mode (plan and execution), not plan mode alone, and plan artifacts) in the active contributor's native language: the person driving the current session, not a fixed project owner. When the SessionStart harness injects a language code, that injection is the authoritative source and MUST NOT be overridden by an English default or by any runtime free-text request. If you notice drafting in another language mid-output, STOP and re-emit in the contributor's language; drift is a defect, not a style choice. If the contributor's language is unknown, use a portable question handoff to ask, then honor the answer; never silently fall back to a project-owner language or to English. If the project lacks contributor-language metadata, prepare it before relying on this rule.

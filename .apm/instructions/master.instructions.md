...
- Every deterministic gate in this section follows one rule: if the gate is missing, build it before the operation it guards — never substitute agent memory for an absent gate. The bullets below apply this rule to specific operations; they name the gate without re-deriving it.
...
- Run expert review/repair agents at one concentrated point, only after the deterministic gates pass — they handle only the semantic judgment determinism cannot, never the artifact code itself (section 1's separate execution agent owns that). Apply the review/repair dispatch of the dispatching-parallel-agents and subagent-driven-development skills here, not their implementation pass.

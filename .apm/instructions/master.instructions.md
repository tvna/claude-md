...
- Sync stale branches by rebase, not by `git merge main`. The squash merge policy hides merge-from-main commits at merge time, but the PR diff and review history carry the noise; a structural preflight (parent count > 1 in `{base}..HEAD`) rejects them at push time. Recovery procedure and the lock-step design rules between the gate and the retro classifier live in [`docs/runbooks/rebase-debt-recovery.md`](docs/runbooks/rebase-debt-recovery.md).

...
- In plan mode, write the plan file in the primary CODEOWNERS owner's native language (resolved by `scripts/plan_language_context.py` via `.github/owners.yaml`).
...
- Keep GitHub posts via `mcp__github__*` ASCII — `scripts/preflight_non_ascii.py` enforces that repository boundary.
...
- In plan mode, write chat responses in the primary CODEOWNERS owner's native language (resolved by `scripts/plan_language_context.py` via `.github/owners.yaml`).

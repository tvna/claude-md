...
- For every open review thread that is contributing to or caused a merge block, post a reply before ending the turn -- regardless of whether the outcome is a code fix, a won't-fix decision, or an acknowledgement. A reply citing the fix commit SHA (when applicable) or stating the disposition closes the review loop visibly; without it, the reviewer cannot tell the comment was seen. Address all such threads in one turn, not one per turn.

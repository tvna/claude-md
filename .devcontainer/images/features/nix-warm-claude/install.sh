...
# Bake the `.#claude` Nix devShell closure into the image at build time so the
# first runtime `nix develop .#claude` no longer realises the closure from
...
# Feature's own directory by the publish workflow (claude legs only) and copied
# next to this script.
...
if ! command -v nix >/dev/null 2>&1; then
  echo "nix-warm-claude: nix not on PATH after sourcing the nix profile" >&2
  exit 69
fi
...
work="/opt/nix-warm-claude"
rm -rf "$work"
mkdir -p "$work"
for f in flake.nix flake.lock pyproject.toml; do
  if [ ! -f "$script_dir/$f" ]; then
    echo "nix-warm-claude: missing staged flake file: $f" >&2
    exit 65
  fi
  cp "$script_dir/$f" "$work/$f"
done

# Realise and leave the full claude devShell closure in /nix/store. The store
...
echo "nix-warm-claude: baked .#claude devShell closure"

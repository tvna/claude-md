...
| 5 | In-session retro ownership (D1) and the CI backstop can both target one PR; dedup relies on CI `find_existing_retro` recognizing the canonical title, else the two retros race. `find_existing_retro` is the sole mechanism preventing duplicate retro creation in `run()`; no other lock or idempotency key exists. In the normal flow the race window is zero: Stop fires at the pre-merge handoff (before the human merges) so CI cannot have started yet. Speculation: in the unusual case where the human merges while the session is still active the race window is small because CI start takes time, but the exact window is not bounded by any constant in this codebase; `find_existing_retro` is the correctness barrier regardless of timing. `find_existing_retro` recognizes both `chore(auto-retro)` (canonical, in-session) and `fix(auto-retro)` (legacy) prefixes; test coverage for the canonical case at the `run()`-level skip path was added in #1916. | `run` skips when `find_existing_retro` matches; `scripts/_auto_retro_render.py:695`; `is_retro_issue_title` at `scripts/_auto_retro_parse.py:322`; CI `open-retro` gated on merged PR; `.github/workflows/post-merge.yml:29-30`. | #1581 #1916 |

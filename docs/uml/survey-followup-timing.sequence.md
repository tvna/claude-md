...
| 5 | In-session retro ownership (D1) and the CI backstop can both target one PR; dedup relies on CI `find_existing_retro` recognizing the canonical title, else the two retros race. In practice the race window is near-zero: in the normal flow Stop fires at the pre-merge handoff (before the human merges) so the in-session retro is created before CI can start; in the unusual case where the human merges while the session is still active the CI runner startup time (~30–60 s) provides a practical buffer that exceeds the in-session creation time (~5 s). Accepted risk (#1916). `find_existing_retro` recognizes both `chore(auto-retro)` (canonical, in-session) and `fix(auto-retro)` (legacy) prefixes; test coverage for the canonical case was missing and added in #1916. | `run` skips when `find_existing_retro` matches; `scripts/_auto_retro_render.py:695`; `is_retro_issue_title` at `scripts/_auto_retro_parse.py:322`; CI `open-retro` gated on merged PR; `.github/workflows/post-merge.yml:29-30`. | #1581 #1916 |

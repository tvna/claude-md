...
| 5 | in-session retro 責務（D1）と CI バックストップが 1 つの PR を両方対象にしうる。dedup は CI `find_existing_retro` が正規タイトルを認識することに依存し、さもないと 2 つの retro が競合する。`find_existing_retro` は `run()` において重複 retro 作成を防ぐ唯一の機構であり、他のロックや冪等性キーは存在しない。通常フローではレース窓はゼロ: Stop は pre-merge ハンドオフ時点（human がマージする前）に発火するため CI は起動不能。Speculation: 異常ケース（agent 稼働中に human がマージ）ではレース窓は小さいと推測されるが、コードベース内にその上限を示す定数は存在しない; タイミングによらず `find_existing_retro` が正確性の砦となる。`find_existing_retro` は `chore(auto-retro)`（正規形式、in-session パス）と `fix(auto-retro)`（旧形式）の両プレフィックスを認識する。`run()` レベルのスキップパスにおける正規形式のテストカバレッジは #1916 で追加した。 | `run` は `find_existing_retro` 一致時に skip; `scripts/_auto_retro_render.py:695`; `is_retro_issue_title` は `scripts/_auto_retro_parse.py:322`; CI `open-retro` はマージ済みPRでゲート; `.github/workflows/post-merge.yml:29-30`。 | #1581 #1916 |

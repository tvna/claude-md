...
| 5 | in-session retro 責務（D1）と CI バックストップが 1 つの PR を両方対象にしうる。dedup は CI `find_existing_retro` が正規タイトルを認識することに依存し、さもないと 2 つの retro が競合する。実際のレース窓は near-zero: 通常フローでは Stop が pre-merge ハンドオフ時点（human がマージする前）に発火し、in-session retro は CI が起動する前に作成済みとなる。異常ケース（agent が稼働中に human がマージ）でも CI runner の起動時間（約 30〜60 秒）が in-session 作成時間（約 5 秒）を大きく上回るため実質的なバッファになる。受容済みリスク（#1916）。`find_existing_retro` は `chore(auto-retro)`（正規形式、in-session パス）と `fix(auto-retro)`（旧形式）の両プレフィックスを認識する。正規形式のテストカバレッジが欠如していたため #1916 で追加した。 | `run` は `find_existing_retro` 一致時に skip; `scripts/_auto_retro_render.py:695`; `is_retro_issue_title` は `scripts/_auto_retro_parse.py:322`; CI `open-retro` はマージ済みPRでゲート; `.github/workflows/post-merge.yml:29-30`。 | #1581 #1916 |

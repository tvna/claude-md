...
Workflow: `dependency-threat-triage` job in [`.github/workflows/weekly-maintenance.yml`](../.github/workflows/weekly-maintenance.yml). Script: [`scripts/threat_intel_triage.py`](../scripts/threat_intel_triage.py). Runbook: [`docs/runbooks/issue-triage.md` Aggregated findings on the security umbrella](../runbooks/issue-triage.md#aggregated-findings-on-the-security-umbrella).
...
- **Rollback path.** Concrete pause/resume, revert, and runaway-issue cleanup commands live in the operator runbook [`docs/runbooks/auto-retrospective-automation.md`](../runbooks/auto-retrospective-automation.md) (sections 5-6). In brief: pause with `gh workflow disable .github/workflows/post-merge.yml --ref main` (resume with `gh workflow enable`), and revert the implementing PR with `git revert <merge-sha>`. Runaway retro issues are identifiable by label `type:docs + layer:meta` plus title prefix `chore(auto-retro)` (legacy retros use `fix(auto-retro)`; Refs #1069).

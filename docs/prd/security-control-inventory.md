...
| `generate-agents.yml` | Persist, LM | Dispatch + reusable compile; weekly schedule is called by `weekly-maintenance.yml`; drift check via `git diff --exit-code` on `CLAUDE.md` / `AGENTS.md`; opens PR rather than push direct. | `.apm/instructions/master.instructions.md`, `verify-apm.yml`, #112 | partially covered | #183 (downstream review checklist) |
...
| Surface | ATT&CK | Existing defense | Evidence | Status | Gap |
|---|---|---|---|---|---|
| `main.json` | IA, Persist, Impact | Default-branch ruleset: 3 required status checks (`Verify agent instructions / gate`, `Verify issue link / gate`, `Verify title policy / gate`); blocks force-push; requires PR + linear history + resolved threads + code-owner review; squash-only merge; blocks deletion. | `docs/runbooks/rulesets.md`, `weekly-maintenance.yml`, #18, #27, #120 | partially covered | #120 (required-checks-vs-ruleset live sync) |
...
| `uv.lock` | RD, Exec, C2 | Locked transitive snapshot; CI uses `uv sync --locked`; Dependabot weekly bumps; OSV + KEV scanning via `issue-pr-triage.yml` / `triage`. | `verify-agents.yml`, `verify-apm.yml`, `issue-pr-triage.yml`, `.github/dependabot.yml` | covered | — |
...
| RD | `verify-github-content.yml`, `issue-pr-triage.yml`, `.github/dependabot.yml`, `uv.lock`, `pyproject.toml`, `dependabot.json` ruleset, `docs/runbooks/agent-provenance.md` | covered | — |
| IA | `main.json`, `all-branches.json`, `dependabot.json`, `verify-github-content.yml`, `weekly-maintenance.yml` | partially covered | #120 |
...
| Persist | `verify-apm.yml`, `weekly-maintenance.yml`, `apply-labels.yml`, `apply-rulesets.yml`, `verify-github-content.yml` | covered | — |
...
| DE | `verify-github-content.yml`, `issue-pr-triage.yml`, `verify-apm.yml`, `preflight_non_ascii.py` | covered | — |

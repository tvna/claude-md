...
| [gitapex-ssot-gate-registry.md](gitapex-ssot-gate-registry.md) | Design PRD for `.gitapex/ssot.json`: a cross-plane gate-function registry with a machine-readable label routing table and policy-source pointers (references, not values, per #1984), governed by CODEOWNERS plus a paired validator gate, with a four-phase blast-radius-ordered migration plan. | #2246, #1984, #1041 | `docs/proposals/config-ssot-duplicate-fact-inventory.md`; `docs/runbooks/issue-triage.md`; `.github/label-policy.toml`; `docs/next-session/gitapex-ssot-phase0.md` |

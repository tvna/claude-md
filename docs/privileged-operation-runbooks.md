...
- **Dry-run command.** A dependabot PR is itself the plan: it changes the lockfile or version pin, runs the full CI suite (verify-agents, verify-apm-drift, verify-apm-portability), and waits for review before merging. There is no separate `dry_run=true` dispatch because the PR-and-CI cycle is the dry-run.
...
- **Audit / post-apply verification.** Dependabot is the audit trail: every dependency update is a PR with diff, CI status, and review history. [`scripts/uv_pin.py`](../scripts/uv_pin.py) `verify-drift` enforces that every literal `UV_VERSION:` reference in `.github/workflows/`, `scripts/`, and `docs/` matches `[tool.uv].required-version`, blocking partial updates.

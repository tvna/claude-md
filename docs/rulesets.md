...
Issue and PR titles are metadata surfaces: they appear in notifications, project views, triage lists, and agent summaries before body-level context is inspected. The title boundary is therefore stricter than the body/comment non-ASCII workflow. Titles must be ASCII-only; Japanese text, emoji, zero-width marks, RTL controls, fullwidth homoglyphs, and other multi-byte control surfaces are rejected by `.github/workflows/verify-title-policy.yml`.
...
3. Edit the title to ASCII-only and confirm the same check passes.
4. Confirm the merge box remains blocked while the failing required check is present.

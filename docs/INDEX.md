...
| [git-push-gate-chain.sequence.md](uml/git-push-gate-chain.sequence.md) | Sequence diagram of the six PreToolUse Bash gates that fire on `git push` (signing-bypass flag, unsigned-commit push, base-currency, session-branch, authz, non-empty) plus the external prek runner, with a gap analysis of fail-open vs fail-closed per gate, the correlated session-branch fail-open pair, the unwired `preflight_push_prek.py`, and the prek-offline degraded path. FTA/FMEA prep. | #2341, #1713, #2138, #785, #1130, #901, #1931 | `scripts/gate_unsigned_commit_bash.py`; `scripts/preflight_push_unsigned_commits.py`; `scripts/preflight_push_base.py`; `scripts/preflight_push_session_branch.py`; `scripts/preflight_session_branch_authz.py`; `scripts/preflight_push_nonempty.py`; `scripts/preflight_push_prek.py` |
| [git-push-gate-chain.sequence.ja.md](uml/git-push-gate-chain.sequence.ja.md) | Japanese translation of `git-push-gate-chain.sequence.md` (owner-language reading copy of the push gate chain sequence and gap analysis). | #2341, #785, #1130, #901, #1931 | `docs/uml/git-push-gate-chain.sequence.md` |
| [pr-body-fix-loop.sequence.md](uml/pr-body-fix-loop.sequence.md) | Sequence diagram of the `create_pull_request -> post_pr_create_body_fix -> update_pull_request` feedback loop, with a gap analysis of the missing cycle-cap gate (convergence rests on PostToolUse matcher scoping, not a counter), the create-vs-update footer asymmetry, the Stop-hook interruption risk, and the absent update-side body verification. FTA/FMEA prep. | #2341, #892, #382, #356 | `scripts/post_pr_create_body_fix.py`; `scripts/preflight_pr_body_required_sections.py`; `scripts/preflight_pr_template_shape.py`; `scripts/pr_body_close_keyword_gate.py` |
| [pr-body-fix-loop.sequence.ja.md](uml/pr-body-fix-loop.sequence.ja.md) | Japanese translation of `pr-body-fix-loop.sequence.md` (owner-language reading copy of the PR body-fix loop sequence and gap analysis). | #2341, #892, #382, #356 | `docs/uml/pr-body-fix-loop.sequence.md` |
| [hook-gate-fta-fmea.gap.md](uml/hook-gate-fta-fmea.gap.md) | FTA/FMEA of the hook/gate architecture: four fault trees (undeserved merge; merge livelock; Stop-hook freeze; self-improvement loop degradation) grounded in file:line evidence, plus a Severity x Detection FMEA table over every major gate. Top findings: the fail-closed merge gate is audit-mode-suppressible (`auditable` default), the advisory poller's 20s budget is reused fail-closed, Stop-hook composition is one-shot and unmodeled, the session-branch common cause now spans four gates, and the retro fallback path is self-contradictory. | #2341, #1932, #1945, #1946, #785, #1513, #901, #1931 | `scripts/gate_merge_safety.py`; `scripts/check_pr_mergeability.py`; `scripts/_hook_runtime.py`; `scripts/gate_stop_pr_review_reply.py`; `scripts/stop_new_session_handoff_prompt.py`; `scripts/gate_decision_handoff_askuserquestion.py`; `scripts/gate_cache_regime_advisor.py`; `scripts/check_session_branch.py`; `scripts/post_merge_retro_append.py`; `scripts/gate_reserved_retro_scope.py` |
| [hook-gate-fta-fmea.gap.ja.md](uml/hook-gate-fta-fmea.gap.ja.md) | Japanese translation of `hook-gate-fta-fmea.gap.md` (owner-language reading copy of the fault trees, FMEA table, and gap analysis). | #2341, #1932, #1945, #1946 | `docs/uml/hook-gate-fta-fmea.gap.md` |
...
typed document dependency graph declared in `.gitapex/doc-dependencies.toml`.
This folder is owned by the post-merge automation
(the `decision-tree` job in `.github/workflows/post-merge.yml`); it is not

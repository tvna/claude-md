...
| [repo-scope.md](standards/repo-scope.md) | Repo purpose statement and the content-based prohibition on agent-tool-specific configuration files (the Q1 disqualifier). | #58 | `.gitignore` (current enforcement; Phase 4 CI gate parked) |
...
| [host-uv-pin.md](runbooks/host-uv-pin.md) | Brew-independent procedure for aligning a developer host's uv with the repository pin: a workspace-local pinned uv plus a `.code-workspace` PATH prepend and `folderOpen` bootstrap task. | #1205 | `claude-md.code-workspace`; `scripts/setup_pinned_uv.sh`; `scripts/uv_pin.py`; `docs/standards/remote-environment.md` |
...
| [remote-session-base-update.md](runbooks/remote-session-base-update.md) | Fully server-side, no-force-push base-update path for stale `claude/*` branches in remote Claude sessions: conflict probe, `refresh_pr_branch.py` fast path for conflict-free cases, and the server-side `create_or_update_file` + `update_pull_request_branch` dance for generated-file conflicts. Also covers owner-only handoffs: final `--admin` squash-merge and the manual `Apply rulesets` dispatch required after a SoT-only ruleset change (remote session cannot `workflow_dispatch`). | #1802, #1775, #1727 | `scripts/preflight_session_base_freshness.py`; `scripts/refresh_pr_branch.py`; `docs/runbooks/refresh-behind-pr.md`; `docs/runbooks/rulesets.md` |

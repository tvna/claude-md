...
| [merge-queue-failures.md](runbooks/merge-queue-failures.md) | Triage guide for GitHub Merge Queue: which required checks trigger on `merge_group`, how PR-body steps are skipped on the integration ref, and how to classify a red queue run. | #895 | `.github/workflows/portable-pr-policy.yml`; `.github/workflows/verify-agents.yml`; `.github/workflows/verify-dependabot-labels.yml`; `.github/workflows/verify-design-philosophy.yml`; `.github/workflows/verify-ruleset-sync.yml`; `.github/rulesets/main.json` |
...
| [update-pr-branch-recovery.md](runbooks/update-pr-branch-recovery.md) | Recovery procedure for when a PR branch falls behind `main`; replaces the blocked `mcp__github__update_pull_request_branch` with a new-branch-from-main approach. | #893 | `scripts/gate_update_pr_branch.py`; `scripts/preflight_pr_single_commit.py` |

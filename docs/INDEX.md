...
| [git-push-gate-chain.sequence.md](uml/git-push-gate-chain.sequence.md) | Sequence diagram of the PreToolUse Bash gate chain that fires on `git push` (unsigned-commit, base-currency, session-branch, authz, non-empty) plus the external prek runner, with a gap analysis of fail-open vs fail-closed per gate, the correlated session-branch fail-open pair, the unwired `preflight_push_prek.py`, and the prek-offline degraded path. FTA/FMEA prep. | #2341, #785, #1130, #901, #1931 | `scripts/gate_unsigned_commit_bash.py`; `scripts/preflight_push_base.py`; `scripts/preflight_push_session_branch.py`; `scripts/preflight_session_branch_authz.py`; `scripts/preflight_push_nonempty.py`; `scripts/preflight_push_prek.py` |

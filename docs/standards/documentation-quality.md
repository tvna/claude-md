...
The blocking gate covers Markdown files that define the repository's
contributor-facing contract:

- Top-level `*.md` files.
- `.github/*.md` templates.
- `docs/INDEX.md` and `docs/agent-provenance.md`.
- `docs/standards/**/*.md`.
- `docs/runbooks/**/*.md`.

The first pass intentionally excludes `docs/prd/`, `docs/archive/`, and
`docs/generated/`. Those lanes contain design records, frozen history,
and generated views; they can be added after their existing link debt is
repaired in focused PRs.

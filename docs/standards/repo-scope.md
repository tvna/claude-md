...
| `.agents/skills/` ([#728](https://github.com/tvna/claude-md/issues/728), [#1572](https://github.com/tvna/claude-md/issues/1572)) | **Narrow directory carve-out** for cross-client skills (`SKILL.md`). Two sources are permitted: (a) APM-deployed skills from pinned dependencies, whose source of truth remains `apm.yml` plus `apm.lock.yaml`; and (b) local first-party skills authored directly in-repo (#1572). Both are cross-client by construction -- Claude, Codex (`apm compile -t agent-skills`), and Devin (`docs/standards/devin-apm-compatibility.md`) all read `SKILL.md` from here, so this is not a tool-specific mirror. Per-tool mirrors such as `.claude/skills/` remain prohibited by `.gitignore`. Local skills carry only reviewed prompt text and are validated by the `skill-quality-gate` pre-commit hook (`waza check spec-compliance`). |

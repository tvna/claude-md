...
| `.claude/settings.json` ([#109](https://github.com/tvna/claude-md/issues/109)) | **Narrow file-level carve-out** to host deterministic lifecycle hooks. The broader `.claude/` directory rule still applies to every other path under `.claude/` (e.g. `.claude/hooks/`). See *Security tradeoff* below. |
| `.claude/commands/` ([#1572](https://github.com/tvna/claude-md/issues/1572)) | **Narrow directory carve-out** to host repo-owned, frequently-used workflow slash commands (`/issue-pr`, `/issue-recheck`, `/ci-rootcause`). Each command is a reviewed prompt that encodes the project workflow (issue-first, plan mode, verification, root-cause-before-fix); no executable shell, credentials, or model config. The broader `.claude/` directory rule still applies to every other path under `.claude/`. See *Security tradeoff* below. |

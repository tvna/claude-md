...
| P4 - safety boundary | Minimum code; safety-bounded simplicity; defense-in-depth preservation; fail-loud over silent default; debug instrumentation as attack surface | `(lint and type gates exist in workflow-script-quality.md M8; behavioral check is reviewer judgment)` | `docs/workflow-script-quality.md` (M1 to M9 must-have checklist; O1 to O7 optional enhancements) | The consumer's own language ecosystem and code style | Embedding a stack-specific example (Python, pytest, pydantic) into universal text |

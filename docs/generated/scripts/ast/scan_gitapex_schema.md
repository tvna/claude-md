...
flowchart TD
    ...
    N002["return sorted(gitapex_dir.glob('<str>'))"]
    N001 -->|"start"| N002
```
...
flowchart TD
    ...
    N007["errors = []"]
    N008["try"]
    N009["for toml_path in toml_files:     errors.extend(verify_file(toml_path, display=f'{args.gitapex_dir}<str>{toml_path.name}'))"]
    N010["except SchemaError"]
    N011["print(...)"]
    N012["return 1"]
    N013["if errors"]
    N014["for message in errors:     print(f'<str>{_SCRIPT}<str>{message}', file=sys.stderr)"]
    N015["return 1"]
    N016["print(...)"]
    N017["return 0"]
    N001 -->|"start"| N002

...
flowchart TD
    ...
    N007["errors = []"]
    N008["for toml_path in toml_files:     try:         errors.extend(verify_file(toml_path, display=f'{args.gitapex_dir}<str>{toml_path.name}'))     except SchemaError as exc:         errors.append(str(exc))"]
    N009["if errors"]

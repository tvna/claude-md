...
flowchart TD
    ...
    N002["out = {}"]
    N003["for number in numbers:     try:         raw = gh_api('<str>', f'<str>{repo}<str>{number}')     except GitHubApiError:         continue     try:         data = json.loads(raw) if raw.strip() else {}     except json.JSONDecodeError:         continue     title = data.get('<str>')     if isinstance(title, str):         out[number] = title"]
    N004["return out"]
    N001 -->|"start"| N002
    N002 --> N003
    N003 --> N004
...
flowchart TD
    ...
    N007["if hours_raw is None or (isinstance(hours_raw, str) and (not hours_raw.strip()))"]
...
flowchart TD
    ...
    N007["if days_raw is None or (isinstance(days_raw, str) and (not days_raw.strip()))"]

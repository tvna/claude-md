...
flowchart TD
    ...
    N018["if schema.get('additionalProperties') is False"]
    N019["for key in instance:     if key not in properties:         errors.append(f'{path}<str>{key!r}')"]
    N020["for key, subschema in properties.items():     if key in instance and isinstance(subschema, dict):         _validate_instance(instance[key], subschema, root, f'{path}<str>{key}', errors)"]
    N021["if isinstance(instance, list)"]
    N022["min_items = get(...)"]
    N023["if isinstance(min_items, int) and len(instance) < min_items"]
    N024["append(...)"]
    N025["item_schema = get(...)"]
    N026["if isinstance(item_schema, dict)"]
    N027["for idx, item in enumerate(instance):     _validate_instance(item, item_schema, root, f'{path}<str>{idx}<str>', errors)"]
    N028["end"]
    N001 -->|"start"| N002
    N002 --> N003
    N003 -->|"true"| N004

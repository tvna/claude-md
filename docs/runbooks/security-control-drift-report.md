...
| `apm-instructions` | `apm compile` + `git diff --exit-code CLAUDE.md AGENTS.md` | `apm` runs as the nix-built native binary pinned by `flake.nix` (`.github/actions/resolve-apm-version` realizes `.#apm-cli` via `nix build`), not via uv, mirroring `verify-pr.yml`. |
...
| `apm-instructions` | Locally run `apm compile` (the nix-built native binary, not via uv) and commit the regenerated `CLAUDE.md` / `AGENTS.md`. |

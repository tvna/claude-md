...
- On SoT-vs-live drift: maintains a single rolling issue titled `fix(ruleset-drift): SoT vs live drift detected` with the unified diff in a collapsible block; labels `layer:p3-harness`, `area:governance`, `type:fix`; body cites `#30` as the parent. The run date moved out of the title into the body so the issue stays stable across runs.

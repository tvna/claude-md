...
| File | Target | Purpose |
|---|---|---|
| `.github/rulesets/main.json` | `~DEFAULT_BRANCH` | Strict `main` protection (PR-only, squash-only, required status check, squash merge queue). Squash-only merges keep history linear without an explicit `required_linear_history` rule, which is incompatible with the merge queue (see below). |
...
**Incompatibility with `required_linear_history`**: a `merge_queue` rule MUST NOT coexist with a `required_linear_history` rule. GitHub rejects the combination with `HTTP 422` and the body `{"errors":["Invalid rule 'merge_queue': "]}` on the ruleset PUT (issue #895). The merge queue forms an integration ref by stacking entries, which the linear-history rule forbids. So `main.json` carries the `merge_queue` rule and drops `required_linear_history`; history still stays linear because `main` is squash-only (`allowed_merge_methods: ["squash"]` plus `merge_method: SQUASH` on the queue) and `non_fast_forward` blocks history rewrites -- the guarantee moves from the explicit rule to squash-only, not removed.

**Strict required status checks under a merge queue**: `strict_required_status_checks_policy` is kept `false` while the `merge_queue` rule is present. The merge queue already builds each entry on top of the current base and runs the required checks on that integration ref, so the strict "require branches up to date" flag is redundant. (This flag was *not* the cause of the apply-time `HTTP 422`; that was `required_linear_history` above.) See [`merge-queue-failures.md`](https://github.com/tvna/claude-md/blob/main/docs/runbooks/merge-queue-failures.md) for the apply-time triage.
...
3. Then dispatch `Apply rulesets` with `ruleset=main, dry_run=true`, confirm the planned PUT diff is the added `merge_queue` rule, the removed `required_linear_history` rule, and `strict_required_status_checks_policy` set to `false` (all three are required together -- see the incompatibility notes above), and re-dispatch with `dry_run=false` (Phase **P6-merge-queue** above).

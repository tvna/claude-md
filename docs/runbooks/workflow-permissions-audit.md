...
| `publish-devcontainer-images.yml` | `push` to `main` with devcontainer/toolchain path filters + `workflow_dispatch` | `GITHUB_TOKEN` | `contents: read`, `packages: write` | `contents: read`, `packages: write` | `none`. Residual: publishes executable devcontainer images to GHCR with `npx @devcontainers/cli@0.86.1`; the CLI version is exact-pinned in workflow env and the images are tagged with both `main` and the source commit SHA. | #664 |

...
| `publish-devcontainer-images.yml` | `push` to `main` with devcontainer/toolchain path filters + `workflow_dispatch` | `GITHUB_TOKEN` | build/publish: `contents: read`, `packages: write`; pin PR: `contents: write`, `pull-requests: write` | build/publish: `contents: read`, `packages: write`; pin PR: `contents: write`, `pull-requests: write` | `none`. Residual: publishes executable devcontainer images and non-runnable BuildKit cache tags to GHCR with `npx @devcontainers/cli@0.86.1`; the CLI version is exact-pinned in workflow env and the runnable images are tagged with both `main` and the source commit SHA. The generated pin commit is authored by `github-actions[bot]` (via `git config user.name/email`); the PR itself is opened by the identity behind `DEVCONTAINER_PIN_PR_TOKEN` (not necessarily `github-actions[bot]`). Auto-merge is requested after creation or reuse. See #850. | #664, #696, #850 |
...
| `verify-pr.yml` / `verify-dependabot-labels` | `pull_request` | none | `contents: read` | `(job: verify-dependabot-labels)` `contents: read` (top-level `permissions: {}`) | `none`. Consolidated in #1319 (the workflow-level path filter was dropped on consolidation; the static check is cheap and now runs on every PR). | — |
...
| `verify-github-content.yml` | `issues` (opened/edited/reopened) | `GITHUB_TOKEN` via `env.GH_TOKEN` | `contents: read`, `issues: read` | `contents: read`, `issues: read` | `none`. PR title/body/link checks moved to the `portable-pr-policy` job in `verify-pr.yml`; this workflow now covers issue title/body shape only. | — |

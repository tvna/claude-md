...
| `publish-devcontainer-images.yml` | `push` to `main` with devcontainer/toolchain path filters + `workflow_dispatch` | `GITHUB_TOKEN`; `DEVCONTAINER_PIN_PR_TOKEN` from the `devcontainer-image-pins` Environment only for generated pin PR creation | build/publish: `contents: read`, `packages: write`; pin PR: `contents: write`, `pull-requests: write` | build/publish: `contents: read`, `packages: write`; pin PR: `contents: write`, `pull-requests: write` | `none`. Residual: publishes executable devcontainer images and non-runnable BuildKit cache tags to GHCR with `npx @devcontainers/cli@0.86.1`; the CLI version is exact-pinned in workflow env and the runnable images are tagged with both `main` and the source commit SHA. Token issuance and rotation steps live in `docs/runbooks/devcontainers.md`. | #664, #696 |
...
| `issue-pr-triage.yml` / `triage` | `issues` (opened/edited/labeled/unlabeled/reopened), `pull_request_target` (opened/edited/synchronize/labeled/unlabeled/reopened/ready_for_review) | `GITHUB_TOKEN` via `env.GH_TOKEN` | `contents: read`, `issues: write` | `contents: read`, `issues: write` | `none`. Residual: `pull_request_target` is limited to same-repository pull requests because the job guard rejects fork PRs before checkout or label mutation. | #170 (sustained ops) |
...
| #181 | Workflow permissions and PAT audit (least privilege matrix) | self-referenced; matrix rows that surface a `mismatch` other than `none`: `generate-agents.yml`, `weekly-maintenance.yml`, `verify-agents.yml`, `verify-github-content.yml` |
...
Surfaces explicitly marked `none` (no follow-up): `post-merge.yml`, `dependabot-automerge.yml`, `issue-pr-triage.yml` / `scan`, `issue-pr-triage.yml` / `triage`, `verify-apm.yml`, `verify-dependabot-labels.yml`.

...
| `publish-devcontainer-images.yml` | `push` to `main` with devcontainer/toolchain path filters + `workflow_dispatch` | `GITHUB_TOKEN`; `DEVCONTAINER_PIN_PR_TOKEN` only for generated pin PR creation | build/publish: `contents: read`, `packages: write`; pin PR: `contents: write`, `pull-requests: write` | build/publish: `contents: read`, `packages: write`; pin PR: `contents: write`, `pull-requests: write` | `none`. Residual: publishes executable devcontainer images and non-runnable BuildKit cache tags to GHCR with `npx @devcontainers/cli@0.86.1`; the CLI version is exact-pinned in workflow env and the runnable images are tagged with both `main` and the source commit SHA. | #664, #696 |

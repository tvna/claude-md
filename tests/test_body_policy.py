...
class TestStripDevinReviewBadge:
    """Tests for :func:`body_policy.strip_devin_review_badge`. Refs #994."""
    ...
    _BADGE_BLOCK = (
        ...
        "---\n"
        "\n"
        '<a href="https://app.devin.ai/review/tvna/claude-md/pull/993"'
        ' target="_blank">\n'
        "  <picture>\n"
        '    <source media="(prefers-color-scheme: dark)"'
        ' srcset="https://static.devin.ai/assets/gh-open-in-devin-review-dark.svg?v=1">\n'
        '    <img src="https://static.devin.ai/assets/gh-open-in-devin-review-light.svg?v=1"'
    ...
    def test_badge_stripped_completely(self) -> None:
        body = f"before\n\n{self._BADGE_BLOCK}\n\nafter"
        result = body_policy.strip_devin_review_badge(body)
        assert "devin-review-badge" not in result
        assert "<a href" not in result
        assert "<picture>" not in result
        assert "before" in result
        assert "after" in result

    def test_no_badge_returns_body_unchanged(self) -> None:
        body = "## Summary\n\ntest body\n"
        assert body_policy.strip_devin_review_badge(body) == body

    def test_footer_detected_after_badge_strip(self) -> None:
        footer = "_Generated by [Devin](https://app.devin.ai/sessions/abc)_"
        body = f"## Summary\n\nx\n\n{self._BADGE_BLOCK}\n\n{footer}\n"
        assert body_policy.verify_pr_agent_attribution_footer(body) == []

    def test_footer_not_masked_by_badge_html(self) -> None:
        body = f"## Summary\n\nx\n\n{self._BADGE_BLOCK}\n"
        errors = body_policy.verify_pr_agent_attribution_footer(body)
        assert errors
        assert "agent-attribution footer" in errors[0]

    def test_requested_by_stripped(self) -> None:
        footer = "_Generated by [Devin](https://app.devin.ai/sessions/abc)_"
        body = f"## Summary\n\nx\n\n{footer}\n\nRequested by: @tvna\n"
        result = body_policy.strip_devin_review_badge(body)
        assert "Requested by" not in result

    def test_footer_passes_when_requested_by_follows(self) -> None:
        footer = "_Generated by [Devin](https://app.devin.ai/sessions/abc)_"
        body = f"## Summary\n\nx\n\n{footer}\n\nRequested by: @tvna\n"
        assert body_policy.verify_pr_agent_attribution_footer(body) == []

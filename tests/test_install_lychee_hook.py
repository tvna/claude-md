...
INSTALL_LYCHEE = REPO_ROOT / "scripts" / "install-lychee.sh"
CLAUDE_SETTINGS = REPO_ROOT / ".claude" / "settings.json"


def _pinned_lychee_version() -> str:
...
def _make_fake_lychee(bin_dir: Path, version: str) -> Path:
    """Stub lychee binary: reports ``lychee <version>`` for --version."""
    fake = bin_dir / "lychee"
    fake.write_text(
        "#!/usr/bin/env bash\n"
        f'if [ "${{1:-}}" = "--version" ]; then echo "lychee {version}"; fi\n'
        "exit 0\n"
    )
    fake.chmod(0o755)
    return fake
...
def test_script_is_executable() -> None:
    assert INSTALL_LYCHEE.exists()
    assert os.access(INSTALL_LYCHEE, os.X_OK)
...
def test_local_environment_is_silent_noop(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / ".local" / "bin" / "lychee").exists()
...
def test_remote_true_reuses_pinned_lychee_without_download(tmp_path: Path) -> None:
    """lychee already on PATH at the pinned version -> idempotent skip, no fetch."""
    version = _pinned_lychee_version()
    bin_dir = tmp_path / "fakebin"
    bin_dir.mkdir()
    _make_fake_lychee(bin_dir, version)
...
def test_session_start_registers_install_lychee() -> None:
    ...
    for group in session_start:
        for handler in group.get("hooks", []):
            ...
            if isinstance(cmd, str):
                commands.append(cmd)
    assert any("scripts/install-lychee.sh" in cmd for cmd in commands)

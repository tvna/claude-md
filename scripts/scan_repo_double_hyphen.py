...
Excluded path prefixes (``_SKIP_PREFIXES``):
    ``.agents/skills/`` and ``.claude/skills/``: the two committed copies of the
        APM-deployed upstream dependency (``obra/superpowers`` pinned in
        ``apm.yml``; deployed to both surfaces per ``apm.lock.yaml``). These
        files must not be edited directly; any such content there originates
        upstream and is outside this repo's fix scope.

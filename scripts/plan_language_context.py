...
def build_context_message(owner: str, iso: str) -> str:
    ...
    return (
        ...
        f"mode, write plan files at /root/.claude/plans/*.md AND chat "
        f"responses in language code '{iso}'. Exception: GitHub posts "
        f"created via mcp__github__* write tools (issues, PRs, "
        f"comments, reviews) MUST remain ASCII/English -- "

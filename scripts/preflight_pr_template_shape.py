...
# Refs #1025, #1427. The remote Claude web harness (CLAUDE_CODE_REMOTE=true,
# the same signal preflight_push_session_branch.py keys off) auto-appends
# exactly one session footer when a PR is *created* via MCP. Only the
# create path is auto-appended -- update_pull_request is not, so the footer
# relaxation below is deliberately create-only. Under those two conditions
# the agent must NOT pre-add a footer (the harness supplies it); any other
# invocation keeps requiring exactly one trailing footer.
#
# The create-vs-update asymmetry no longer forces a manual repair: the
# create-path PostToolUse fixer (post_pr_create_body_fix.py) lifts the
# harness-appended footer out of the stored body and re-appends it to the
# normalized body it tells the agent to resend via update_pull_request, so
# the mandated update already carries the footer this gate requires. The
# deny reason below still names the asymmetry for any *standalone* update
# (one not driven by that fixer) so the fix is a single, self-correcting
# step rather than a confusing generic footer error.
#
...
def decide(
    ...
    *,
    ...
    # Refs #1427: name the create-vs-update asymmetry on the update path
    # under the web harness so a missing footer reads as a single,
    # self-correcting step instead of a generic error.
    if (
        canonical == "mcp__github__update_pull_request"
        and _claude_web_harness(environ)
        and any("agent-attribution footer" in e for e in errors)
    ):
        reason += (
            "\n\nNote (web harness): create_pull_request auto-appends the "
            "session footer, but update_pull_request does not, so this "
            "update body must carry exactly one trailing footer line "
            "(e.g. `_Generated by [Claude Code](<session-url>)_`). If this "
            "update was suggested by post_pr_create_body_fix.py, resend its "
            "NORMALIZED BODY verbatim -- that fixer already carries the "
            "harness footer forward."
        )

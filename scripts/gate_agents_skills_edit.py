...
# dependencies, regenerated by ``apm compile``) is the
...
# apm dependency pins (content-addressed commit refs in apm.yml; originally
# just ``obra/superpowers``, now also ``tvna/clairvoyance`` per #2185, and any
# future dependency). A managed tree changes only when one of those pins
# changes, so the gate compares the RESOLVED set of pins at base vs head and
# passes only when that set actually moved. Comparing the resolved pins (not
# mere presence of apm.yml in the diff) is deliberate: apm.yml also carries
# unrelated fields (version, MCP config), so a managed-tree hand edit bundled
# with an incidental apm.yml edit must still fail (Codex review on #2098). A
# pin bump + ``apm compile`` (the documented change path in
# docs/standards/apm-managed-paths.md) moves a pin and passes. This is the
...
def _apm_pins(ref: str, *, runner=subprocess.run) -> frozenset[str] | None:
    """Return the set of ``owner/repo#ref`` apm dependency pins at *ref*, or None.
...
def evaluate_pr(managed: frozenset[str], pin_changed: bool) -> tuple[int, list[str]]:
    ...
    Pass when no managed prefix changed, or when an apm dependency pin
    ...
    return 1, [
        ...
        f"edited without an apm dependency pin change. Changed managed paths: {pretty}. "
        f"These files are generated by `apm compile` from the apm dependency pins in "
        f"{_PIN_FILE}, so a change here with no change to any of those pins is a hand edit "
        "the next compile silently overwrites. Revert these paths; to change skill content, "
        "bump the relevant pin and run `apm compile` (see docs/standards/apm-managed-paths.md). "
        "Refs #2098, #2066, #1892, #2185."
    ]
...
def _cmd_verify(args: argparse.Namespace) -> int:
    ...
    # Only resolve the pins when a managed tree changed; the pins are
    # irrelevant otherwise and skipping the two git shows keeps the common
    # case cheap.
    pin_changed = False
    if managed:
        base_pins = _apm_pins(base)
        head_pins = _apm_pins("HEAD")
        pin_changed = base_pins is not None and head_pins is not None and base_pins != head_pins
    code, errors = evaluate_pr(managed, pin_changed)
    if code == 0:
        if managed:
            print(
                "OK: managed trees changed with an apm dependency pin bump "
